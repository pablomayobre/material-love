...
ripple.draw = function (self,...)
    ...
    for i=1, #self.ripples do
        ...
        if self.ripples[i].finished then
            self.custom()
        else
...
ripple.stencil = function (x, y, w, h, time)
    return {
        ...
        start = function (self, xa, ya, collapse)
            local xw, yh = xa - self.x, ya - self.y
            local wa = math.max(self.w - (xw), xw)
            local ha = math.max(self.h - (yh), yh)
            local ra = (ha * ha + wa * wa) ^ 0.5
            ...
            self.update = function (self, dt)
                ...
                self.stencil = function ()
                    love.graphics.circle("fill", xa, ya, r)
                end
